...
export function PromptSearch({ query, onQueryChange, onClearSearch, recentSearches, popularPrompts }: PromptSearchProps) {
  ...
  const showRecent = focused && !query && showDropdown && (recentSearches.length > 0 || popularPrompts.length > 0);
        ...
        <div className="absolute left-0 right-0 top-full mt-2 z-50 rounded-xl border border-border-subtle bg-elevated shadow-cx-lg animate-fade-in overflow-hidden">
          ...
          {popularPrompts.length > 0 && (
            <div className="border-t border-border-subtle p-3">
              <p className="text-2xs font-semibold uppercase tracking-wider text-text-tertiary mb-2">Popular prompts</p>
              <div className="space-y-0.5">
